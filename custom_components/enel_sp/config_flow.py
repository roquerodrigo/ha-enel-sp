...
from .options_flow import EnelSpOptionsFlow

if TYPE_CHECKING:
    from collections.abc import Mapping

    from .data import EnelSpConfigData, EnelSpConfigEntry


def _credentials_schema(default_username: str | None = None) -> vol.Schema:
    """Monta o schema de usuário/senha, opcionalmente pré-preenchido."""
    return vol.Schema(
        {
            vol.Required(
                CONF_USERNAME,
...
class EnelSpFlowHandler(config_entries.ConfigFlow, domain=DOMAIN):
    """Config flow da Enel São Paulo."""
    ...
    def async_get_options_flow(
        config_entry: EnelSpConfigEntry,  # noqa: ARG004
    ) -> EnelSpOptionsFlow:
        """Retorna o handler do options flow."""
        return EnelSpOptionsFlow()

    # O parâmetro estreitado para ``EnelSpConfigData`` é intencional — a classe
    # base do HA declara ``dict[str, Any] | None`` aqui, e trocamos a conformidade
    # estrita com o LSP por uma tipagem mais forte do nosso schema de user_input.
    async def async_step_user(  # type: ignore[override]
        self,
        user_input: EnelSpConfigData | None = None,
    ...
    async def async_step_reauth_confirm(
        ...
        existing = cast("EnelSpConfigData", entry.data)
    ...
    async def _validate(
        self,
        user_input: EnelSpConfigData,
    ) -> dict[str, str]:
        """Testa as credenciais e retorna um dict de erros (vazio se houver sucesso)."""
        try:
            ...
            return {"base": "auth"}
        except EnelSpApiClientCommunicationError as exception:
