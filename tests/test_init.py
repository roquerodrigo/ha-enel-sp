...
from custom_components.enel_sp import (
    async_reload_entry,
...
async def test_setup_entry_gives_the_client_a_dedicated_session(
    ...
    with (
        patch(
            "custom_components.enel_sp.async_create_clientsession",
            return_value=session,
        ) as create_session,
        patch("custom_components.enel_sp.EnelSpApiClient") as client_class,
    ):
...
async def test_remove_device_refuses_an_installation_the_portal_lists(
    hass, setup_integration
):
...
async def test_remove_device_allows_an_installation_the_portal_dropped(
    hass, setup_integration
):
    stale = dr.async_get(hass).async_get_or_create(
        config_entry_id=setup_integration.entry_id,
        identifiers={(DOMAIN, f"{setup_integration.entry_id}_0100000001")},
    )
