...
TRANSLATIONS_DIR = (
    Path(__file__).parent.parent / "custom_components" / "enel_sp" / "translations"
)
...
async def test_entity_translation_keys_and_authored_names_agree(
    ...
    """
    Toda translation key pedida por uma entidade está escrita, e vice-versa.

    A leitura é feita no entity registry, e não nas classes: o Home Assistant reescreve
    `_attr_translation_key` como uma propriedade, então inspecionar o atributo da classe
    encontra um descriptor e, em silêncio, não verifica nada. Comparar os locales entre
    si também não pega isso — uma chave ausente dos dois lados bate, e a entidade
    recorre ao nome da device class enquanto o nome escrito nunca aparece.
    """
    ...
    requested = {
        ...
        if entry.platform == "enel_sp" and entry.translation_key is not None
    }
